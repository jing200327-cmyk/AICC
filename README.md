...
<p align=center>
  ...
  <strong>龙星行日报</strong> · <strong>任务记录</strong> · <strong>配置中心</strong>
</p>
...
任务记录统一展示线索导入、预分割、测试外呼、正式外呼和日报任务，支持按任务类型及时间筛选，并查看任务状态、执行结果、日志和输出文件。
...
| 层级 | 技术与职责 |
| --- | --- |
| 前端 | 单页 HTML 工作台，使用 Vue 3 与 Element Plus，通过 FastAPI 静态路由访问 |
| API 服务 | FastAPI，负责路由装配、结构化错误、文件下载和任务查询 |
| 适配层 | lead_import、split_import、outcall、daily_report、task_records、config_center |
